...
const dates = ref(JSON.parse(JSON.stringify(props.dates)));
...
async function filter() {
    ...
    await router.get('riports', {
        ...
        replace: true,
        onSuccess: (page) => {
            dates.value = page.props.dates;
            if (dates.value.data.length < 1) {
...
const deleteDate = async (deletedDate) => {
    try {
        ...
        //props.dates.value.data = props.dates.value.data.filter(date => date.id !== deletedDate.id);
        location.reload();        
        closeDateModal();
    } catch (error) {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Riports</h2>
        </template>
